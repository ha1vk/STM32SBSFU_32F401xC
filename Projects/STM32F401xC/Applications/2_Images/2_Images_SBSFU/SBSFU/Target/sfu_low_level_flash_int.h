...
/*
 * STM32F4: The write access type (byte, half-word, word or double word) must correspond to the type of parallelism
 * chosen (x8, x16, x32 or x64)
 */
typedef uint8_t SFU_LL_FLASH_write_t;
...
/* External variables --------------------------------------------------------*/
extern uint32_t FlashSectorsAddress[];

/* Exported macros -----------------------------------------------------------*/
/**
  * @brief Macro to make sure the FWIMG slots are properly aligned with regards to flash constraints
  * Each slot should start at the beginning of a page/sector
  */
#define IS_ALIGNED(address) (0U == ((address) % 2048))
